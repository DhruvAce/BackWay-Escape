    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                // Metallic / AO
                float metallic = SampleTriplanar(_MetallicMap, worldPos, worldNormal).r;
                float ao = SampleTriplanar(_OcclusionMap, worldPos, worldNormal).g;

                // Height (optional bias feel)
                float height = SampleTriplanar(_HeightMap, worldPos, worldNormal).r;
                albedo.rgb *= (1 + height * _HeightStrength);

                // Simple lighting (URP friendly minimal PBR approximation)
